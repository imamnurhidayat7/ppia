...
import { requestWorkspaceTransition } from './workspace-transition';
import {
  ADMIN_NAV,
  MEMBER_NAV,
  filterSections,
...
/**
 * WorkspaceSwitcher — single-button mode toggle.
 *
 ...
 *
 * On click we ask the workspace-transition overlay (mounted at the dashboard
 * root) to show itself, then push the route. The overlay survives the route
 * change because it lives above the switch in the tree — without that, the
 * transition dies the instant this shell unmounts.
 */
function WorkspaceSwitcher({ view }: { view: 'member' | 'admin' }) {
  ...
  const handleToggle = () => {
    requestWorkspaceTransition(nextMode);
  ...
  return (
    <button
      ...
      className={cn(
        ...
        'border-white/15 bg-white/[0.06] text-white/90 hover:border-[#E8231A]/40 hover:bg-white/10 hover:text-white'
      )}
    >
      ...
      >
        ...
        >
          <div className="flex shrink-0 items-center gap-3 sm:gap-6">
            ...
            {/* Hidden below xl — the mobile menu carries a member/admin pill for
                admins, so the desktop switcher doesn't need to fit in the bar. */}
            {isAdmin && (
              <div className="hidden xl:block">
                <WorkspaceSwitcher view={view} />
              </div>
            )}
          </div>
                ...
                {(['member', 'admin'] as const).map((target) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (current) return;
                        // Use the same overlay as the desktop switcher so the
                        // transition feels consistent across breakpoints.
                        requestWorkspaceTransition(target);
                        setMobileMenuOpen(false);
                        router.push(target === 'admin' ? '/dashboard/admin' : '/dashboard');
                      }}
