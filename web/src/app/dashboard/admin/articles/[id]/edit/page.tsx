...
export default function EditArticlePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      showError(errorMessage(error, 'Could not load article'));
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess('Post changes saved');
      router.push('/dashboard/admin/articles');
    } catch (error) {
      showError(errorMessage(error, 'Could not update article'));
      setSubmitting(false);
    }
  };
  ...
  return (
    <PageStack>
      <PageHeading
        eyebrow="Content"
        title="Edit post"
        description={formData.title || 'Update the details of an existing post.'}
        icon={FileText}
